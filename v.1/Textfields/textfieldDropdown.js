...
class TextfieldDropdown extends Component {
    constructor(props) {
        super(props);
        ...
        return (
            <div>
                <button
                    ...
                    aria-haspopup="true" aria-expanded={this.state.isExpanded} aria-controls="dropdown-button"
                    className={"btn btn-secondary dropdown-toggle " + this.props.theme} id={"dropdown-button" + this.state.buttonID}
                ...
                </button>
                {
                    this.state.showMenuState
                        ? (
                            <div
                                role="menu" aria-live="polite"
                                className={"dropdownMenu " + this.props.theme} id={"dropdown-button " + this.props.theme} onClick={this.updateButton}>
                                {
                                    this.props.options.map(function (option, index) {
                                        if (option[1]) {
                                            return <button className="dropdown-item dropdown-link current-selection" key={index}>{option}</button>
                                        } else {
                                            return <button className="dropdown-item dropdown-link" key={index}>{option}</button>
                                        }
                                    })
                                }
                            </div>
